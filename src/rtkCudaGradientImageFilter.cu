...
__global__ void
kernel_GradientProject(float * dev_proj_in, float * dev_proj_out, float * dev_vol, cudaTextureObject_t * dev_tex_vol)
{
  ...
  float* dev_out)
{
  ...
  case 2:
  {
    ...
    size_t pitch;
    cudaMallocPitch(&imgArray, &pitch, cSize[1]*sizeof(float), cSize[0]);
    cudaMallocArray(&imgArray, &channelDesc, cSize[0], cSize[1]);
    cudaMemcpy2D(imgArray, pitch, dev_in, cSize[1]*sizeof(float), cSize[1]*sizeof(float), cSize[0], cudaMemcpyHostToDevice);
    CUDA_CHECK_ERROR;
